...
<Benchmark
  ...
  rows={[
    ...
    { values: ["Warp Shuffle", "30.1", "latency: ~1 cycle per warp", "1.5x"], highlight: true },
...
__global__ void warp_scan(float *input, float *output, int n) {
    ...
    // The up-sweep phase above produces the inclusive scan result directly.
    // Each lane now holds the sum of all elements from lane 0 to lane_id.
