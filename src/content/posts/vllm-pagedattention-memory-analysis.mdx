...
PagedAttention divides GPU memory into fixed-size **blocks** (typically 16 tokens x num_heads x head_dim x 2 [K+V] x dtype_size). Each sequence maintains a **block table**--a mapping from logical token positions to physical block indices.
...
class CopyOnWriteHandler:
    ...
    def write_block(self, seq_id: int, block_idx: int, 
        ...
        if self.ref_counts[old_block] over 1:
...
The default of 16 tokens balances these factors. For long-context workloads (over 32K tokens), larger blocks (32-64) may improve kernel efficiency at the cost of higher internal fragmentation.
...
- **L2 Cache Hit Rate**: Should be over 80% with warm blocks
- **HBM Bandwidth Utilization**: Target over 70% during decode
