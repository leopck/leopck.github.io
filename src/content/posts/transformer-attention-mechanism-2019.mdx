...
**Time Complexity**: O(n^2 x d) where n is sequence length, d is model dimension
...
<Benchmark
  ...
  rows={[
    { values: ["128", "2.1", "0.13", "O(n^2)"], highlight: false },
    { values: ["512", "12.4", "2.1", "O(n^2)"], highlight: false },
    { values: ["1024", "45.2", "8.4", "O(n^2)"], highlight: true },
    { values: ["2048", "178.3", "33.6", "O(n^2)"], highlight: false },
    { values: ["4096", "712.8", "134.2", "O(n^2)"], highlight: false },
  ]}
/>
...
def analyze_attention_memory(n, d, num_heads=12, dtype_bytes=2):
    ...
    """
    # Q, K, V projections: 3 x n x d x dtype_bytes
    qkv_memory = 3 * n * d * dtype_bytes
    
    # Attention scores matrix: n x n x dtype_bytes
    scores_memory = n * n * dtype_bytes
    
    # Attention weights (after softmax): n x n x dtype_bytes
    weights_memory = n * n * dtype_bytes
    
    # Output: n x d x dtype_bytes
...
Memory per head: Still O(n^2), but computation can be parallelized across heads.
...
<Callout type="info" title="KV Cache Benefits">
  KV caching reduces computation from O(n^2) to O(n) per token during autoregressive generation, but requires O(n) memory to store cached keys and values.
</Callout>
...
def batch_attention_memory(batch_size, seq_len, d_model):
    """
    Memory scales with batch_size x seq_len^2
    """
    # Attention matrix per batch: batch_size x seq_len x seq_len
...
2. **Memory scales as O(n^2)** for attention matrices
