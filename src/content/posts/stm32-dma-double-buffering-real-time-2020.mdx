...
This post lays out a **double-buffered DMA** pattern you can reuse for ADC sampling, I2S audio capture/playback, SPI sensor bursts, and UART high-rate logging.

## The problem: CPU-driven IO does not scale
...
At 48 kHz audio, you have **20.8 us per sample**. With any DSP, filtering, or compression, you will get crushed by overhead.
...
## Concrete example: ADC at 200 kS/s with 1024-sample buffer
...
Example: $F_s=200k$, $N=1024$ gives $T_{block}=2.56$ ms. At $f_{cpu}=168$ MHz that gives $C_{avail} \approx 430k$ cycles.
...
<Benchmark
  ...
  rows={[
    { values: ["ADC capture at 200 kS/s", "High CPU, jitter", "Low CPU, stable", "Often 5-20x less overhead"], highlight: true },
    { values: ["I2S audio at 48 kHz", "ISR-heavy", "Block processing", "Transforms impossible into routine"], highlight: true },
  ]}
/>
...
- **Buffer size**: larger means fewer IRQs, more latency
...
Double-buffered DMA is the most reusable real-time optimization pattern on STM32. It turns per-sample work into **block work**, reduces jitter by **removing CPU from the IO path**, and makes throughput predictable because you can **cycle-budget** it. Use it anywhere you want sustained performance without surprises.
