---
title: "Achieving Sub-10uA Sleep Current on ESP32: Register-Level Analysis"
author: "stanley-phoong"
description: "A systematic investigation into ESP32 power domains, RTC memory retention, and peripheral leakage. Includes register-level configurations and oscilloscope measurements proving sub-10uA deep sleep."
...
The ESP32 datasheet claims 10uA deep sleep current. In practice, most developers see 150-500uA. This gap stems from undocumented power domains, peripheral leakage, and default configurations optimized for quick wake-up rather than minimum power. Let's fix that.

## Power Domain Architecture

The ESP32 contains multiple independently-controllable power domains:

{/* TODO: Fix MemoryLayout - non-hex addresses need conversion to layout= format */}
<MemoryLayout
  title="ESP32 Power Domain Hierarchy"
  regions={[
    { start: "Domain 0", end: "", name: "Digital Core", size: "~25mA active", color: "red", notes: "CPU, caches, SRAM" },
    { start: "Domain 1", end: "", name: "RTC Fast Memory", size: "~500uA retention", color: "orange", notes: "8KB, code execution" },
    { start: "Domain 2", end: "", name: "RTC Slow Memory", size: "~2uA retention", color: "green", notes: "8KB, variable storage" },
    { start: "Domain 3", end: "", name: "RTC Peripherals", size: "~150uA", color: "purple", notes: "ULP, touch, ADC" },
    { start: "Domain 4", end: "", name: "RTC Core", size: "~6uA", color: "blue", notes: "RTC timer, wakeup" },
  ]}
/>

The critical insight: **default deep sleep keeps Domain 1 (RTC Fast Memory) powered** for fast wake-up. This alone adds 500uA.
...
### Step 1: Disable RTC Fast Memory (Save ~500uA)
...
void configure_rtc_memory_powerdown() {
    ...
    // This prevents ULP program execution but saves ~500uA
...
### Step 2: Disable RTC Peripherals (Save ~150uA)
...
<Callout type="danger" title="External Pull-ups">
  External pull-up resistors on GPIOs configured as inputs with internal pull-down create a resistor divider drawing continuous current. A 10kΩ pull-up to 3.3V draws 165uA per GPIO!
</Callout>
...
<Benchmark
  ...
  rows={[
    { values: ["Default deep sleep", "147.3 uA", "280ms"], highlight: false },
    { values: ["+ Disable Fast Mem", "42.1 uA", "350ms"], highlight: false },
    { values: ["+ Disable Slow Mem", "28.4 uA", "380ms"], highlight: false },
    { values: ["+ Disable RTC Periph", "12.8 uA", "420ms"], highlight: false },
    { values: ["+ GPIO Isolation", "8.7 uA", "420ms"], highlight: true },
    { values: ["+ 32kHz XTAL", "6.9 uA", "420ms"], highlight: true },
  ]}
  notes="ESP32-WROOM-32, VDD=3.3V, 25 degC ambient"
/>
...
If you're still above 10uA, check these sources:

1. **PSRAM** (if equipped): Adds 20-40uA in retention mode
2. **USB-UART bridge**: CP2102/CH340 can draw 1-5mA from 3.3V rail
3. **LDO quiescent current**: AMS1117 draws 5mA; use HT7333 (3uA)
4. **LED power indicator**: 1-20mA depending on resistor value
5. **External sensors**: I2C devices often have 1-100uA standby draw
...
- Sharp drop from ~40mA to ~10uA over ~100ms
...
For truly battery-powered applications, consider the ESP32-S2 or ESP32-C3 which achieve 5uA deep sleep with default configurations.
