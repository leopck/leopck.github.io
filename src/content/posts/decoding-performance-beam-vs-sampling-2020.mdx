...
<Benchmark
  ...
  rows={[
    { values: ["Sampling (top-k / top-p)", "1", "partial sort + RNG", "1x cache"], highlight: true },
    { values: ["Greedy", "1", "argmax", "1x cache"], highlight: false },
    { values: ["Beam search (B beams)", "B", "top-k + beam scoring", "~Bx cache (if duplicated)"], highlight: true },
  ]}
/>
...
If you duplicate the KV cache per beam, memory grows ~Bx:
...
- $B$ = number of beams
- $S$ = generated tokens
- $T_1$ = time per token for batch=1
...
<Benchmark
  ...
  rows={[
    { values: ["Sampling (top-p)", "1", "12.5", "80", "1.0x"], highlight: true },
    { values: ["Beam search", "4", "18.0", "55", "3.2x"], highlight: false },
    { values: ["Beam search", "8", "26.0", "38", "6.1x"], highlight: false },
  ]}
/>
...
<PerfChart
  ...
  chartData={{
    ...
    series: [{
      label: "ms/token",
      values: [12.5, 14.0, 18.0, 26.0, 45.0],
      color: "#ef4444",
    }]
  }}
  type="line"
...
- Run top-k on GPU (avoid device-to-host sync)
