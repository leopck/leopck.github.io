...
<Benchmark
  title="Stack Layer Optimization Priority"
  columns={["Priority", "Layer", "Technique", "Expected Impact"]}
  rows={[
    { values: ["1 (highest)", "Forward Pass", "FP8 quantization", "2x throughput, 50% latency reduction"], highlight: true },
    { values: ["2", "Forward Pass", "Tensor parallelism (TP=8)", "8x faster prefill, 8x HBM bandwidth"], highlight: true },
    { values: ["3", "Model Runner", "CUDA graphs", "20% decode latency reduction"], highlight: false },
    { values: ["4", "Scheduler", "Chunked prefill", "3-5x better tail ITL"], highlight: false },
    { values: ["5", "Router", "KV-aware routing", "20-40% prefix cache hit rate"], highlight: false },
    { values: ["6", "Attention", "FP8 KV cache", "2x more concurrent requests"], highlight: false },
    { values: ["7", "Gateway", "TLS 0-RTT", "1-2ms TTFT reduction"], highlight: false },
  ]}
/>

Every millisecond in this trace maps to a specific software component and hardware resource. Optimizing the inference stack in 2026 means knowing which component owns which milliseconds and applying the right technique at each layer: faster networks at the gateway, smarter routing at the router, better scheduling in the engine, faster kernels on the GPU, and efficient streaming on the return path. The GPU forward pass dominates at 91% of TTFT, making kernel-level optimizations (quantization, attention backends, TP) the highest-impact investments. But the remaining 9% -- scheduling, routing, sampling -- determines tail latency behavior and can make the difference between a 1.2x and 15x P99/P50 ratio.
