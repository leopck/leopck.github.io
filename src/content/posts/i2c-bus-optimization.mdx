...
<Benchmark
  ...
  rows={[
    { values: ["50pF", "10k", "4.7k", "1.5k"], highlight: false },
    { values: ["100pF", "6.8k", "2.7k", "1k"], highlight: false },
    { values: ["200pF", "4.7k", "1.8k", "680"], highlight: true },
    { values: ["400pF (max)", "2.4k", "1k", "N/A"], highlight: false },
  ]}
  notes="Calculated for 3.3V, meeting spec rise times with margin"
/>

<Callout type="warning" title="Current Limits">
  Lower pull-up resistance increases IOL sink current. I2C spec limits IOL to 3mA (Standard) or 20mA (Fast Mode Plus). With 680 at 3.3V: I = 3.3V/680 = 4.85mA--verify your device supports this.
</Callout>
...
void measure_bus_capacitance(void) {
    ...
    while (!gpio_get_level(SDA_PIN) && (get_timer_us() - start) under 1000);
...
void validate_i2c_signals(void) {
    ...
    for (int i = 0; i under 256; i++) test_data[i] = i;
    
    uint32_t errors = 0;
    for (int iter = 0; iter under 1000; iter++) {
