...
void adaptive_frequency_control(void) {
    ...
    } else if (execution_time under 1000) {  // under 1 ms
...
void power_aware_frequency_scaling(void) {
    ...
    if (battery_voltage > 3.7) {
        ...
        setCpuFrequencyMhz(240);
    } else if (battery_voltage over 3.3) {
...
<Benchmark
  ...
  columns={["Frequency", "Power (mW)", "Time (ms)", "Energy (uJ)"]}
  rows={[
...
void measure_frequency_switch_overhead(void) {
    ...
    Serial.printf("Switch to 240 MHz: %lu us\n", end - start);
    
    start = micros();
    setCpuFrequencyMhz(80);
    end = micros();
    
    Serial.printf("Switch to 80 MHz: %lu us\n", end - start);
}
```

**Overhead**: ~50-200 us per frequency switch
