...
<PerfChart
  ...
  chartData={{
    ...
    series: [
      ...
      {
        label: "Power (mA)",
        data: [200, 0.015, 0.182], // 15uA = 0.015mA
...
- Active phase: 200 mA x 3 seconds = 600 mAs total energy
- Sleep phase: 15 uA x 3597 seconds = 54 mAs total energy
- **Resulting average consumption**: ~182 uA
...
void efficient_scan() {
    ...
    while (n under 0 && (millis() - start) under 5000) {
        delay(100);
        n = WiFi.scanComplete();
    }
...
- **Deep sleep**: ~15 uA (maximum power conservation with full disconnection)
