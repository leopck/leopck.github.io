...
<Benchmark
  ...
  rows={[
    ...
    { values: ["Deep Sleep", "10-150 uA", "~200 ms", "No", "No", "RTC Only"], highlight: true },
    { values: ["Hibernation", "5-10 uA", "~1 s", "No", "No", "None"], highlight: false },
  ]}
/>
...
<Callout type="info" title="Light Sleep Use Cases">
  Light sleep is ideal for applications that need to wake frequently (under 1 second) while maintaining state. Perfect for sensor polling with short intervals.
</Callout>
...
**Measured current**: 10-150 uA (varies based on RTC memory retention and peripherals)
...
void optimize_deep_sleep() {
    ...
    // RTC slow memory: ~2 uA retention
    // RTC fast memory: ~500 uA retention
}
```
...
<Benchmark
  ...
  rows={[
    ...
    { values: ["Deep Sleep (optimized)", "15 uA", "133,333 hours (15 years)"], highlight: true },
  ]}
/>
...
**Power profile**:
- Active (measurement + transmit): 200 mA x 2 seconds = 400 mAs
- Deep sleep: 15 uA x 3598 seconds = 54 mAs
- **Average current**: ~127 uA
