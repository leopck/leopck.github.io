...
```
KV cache = 2 x num_heads x head_dim x dtype_bytes
         = 2 x 32 x 128 x 2 = 16 KB (for 32 heads, FP16)
```
...
**Memory per token per layer:**
```
KV cache = 2 x 1 x head_dim x dtype_bytes
         = 2 x 1 x 128 x 2 = 512 bytes (32x reduction!)
```
...
- Memory per token: 2 x 8 x 128 x 2 = 4 KB
...
```
KV cache = kv_latent_dim + rope_dim x num_heads
         = 512 + 64 x 128  # Example values
