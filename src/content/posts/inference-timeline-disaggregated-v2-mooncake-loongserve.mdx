...
<Benchmark
  title="Failure Recovery Time: Co-located vs Disaggregated"
  columns={["Failure Type", "Co-located Recovery", "Disaggregated v2 Recovery", "Reason"]}
  rows={[
    { values: ["Worker crash (mid-decode)", "Full recompute from prompt", "Resume from KV store", "KV cache survives worker failure"], highlight: true },
    { values: ["OOM during batch growth", "Preempt + recompute", "Spill KV to CPU/NVMe tier", "MemServe tiered storage"], highlight: true },
    { values: ["Network partition", "Requests on partition fail", "Route to other partition", "KV store has replicas"], highlight: false },
    { values: ["GPU hardware error", "Cold restart instance", "Reassign to spare GPU", "Stateless workers + persistent KV"], highlight: true },
  ]}
/>
