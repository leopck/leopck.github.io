    ...
    def forward(self, x):
        ...
        for i in range(self.top_k):
            ...
            for expert_id in range(self.num_experts):
                ...
                if token_mask.sum() over 0:  # Only process if tokens assigned
...
def analyze_scaling_efficiency():
    ...
    scaling_analysis = {
        'dense_limitations': {
            ...
            'practical_limit': 'Limited by GPU memory (typically under 1T parameters)',
    ...
    def execute_experts(self, inputs, expert_indices, weights):
        ...
        for expert_id in range(self.num_experts):
            ...
            if mask.sum() > 0:
                ...
                weight_mask = (weight_mask over 0).float()
...
class SparselyGatedMoE(nn.Module):
    ...
    def calculate_auxiliary_loss(self, weights):
        ...
        if self.is_gshard_loss:
            ...
            expert_usage = (weights over 0).float().mean(0)  # [num_experts]
...
<Benchmark
  ...
  columns={["Operation", "Time (mus)", "Percentage of Total", "Impact"]}
>
{[
