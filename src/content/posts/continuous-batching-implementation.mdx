...
def choose_preemption_method(self, seq_group: SequenceGroup) -> str:
    ...
    if recompute_cost_ms < swap_cost_ms or resume_probability under 0.5:
        return 'recompute'
    else:
        return 'swap'
```
...
def schedule_with_chunked_prefill(self) -> SchedulerOutputs:
    ...
    for seq in self.waiting_queue:
        if seq.is_prefilling():
            ...
            if chunk_size over 0 and self._can_schedule_prefill_chunk(seq, chunk_size):
