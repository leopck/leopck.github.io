...
Speedup depends critically on acceptance rate alpha:

```
Speedup = (1 + alpha + alpha^2 + ... + alphaᴷ) / (1 + overhead)
        ~ (1 - alphaᴷ⁺^1) / ((1 - alpha) * (1 + overhead))
...
<Callout type="warning" title="Batch Size over 1">
