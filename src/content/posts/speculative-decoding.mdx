    ...
    while not should_stop(generated):
        ...
        for i, draft_token in enumerate(draft_tokens):
            ...
            if random.random() < acceptance_prob:
                ...
                generated.append(sample(adjusted))
                break
...
Speedup depends critically on acceptance rate alpha. For K speculative tokens with per-token acceptance rate alpha:

```
Speedup ~ (1 - alpha^(K+1)) / ((1 - alpha) * (1 + overhead))

For K=4, alpha=0.8: Speedup ~ 2.95x (before overhead)
For K=4, alpha=0.5: Speedup ~ 1.56x (before overhead)
```
...
<PerfChart
  ...
  unit="x"
  data={[
    { label: "alpha = 0.9", value: 3.44, color: "green" },
    { label: "alpha = 0.8", value: 2.95, color: "green" },
    { label: "alpha = 0.7", value: 2.50, color: "blue" },
    { label: "alpha = 0.6", value: 2.09, color: "orange" },
    { label: "alpha = 0.5", value: 1.72, color: "orange" },
    { label: "alpha = 0.4", value: 1.41, color: "red" },
  ]}
/>
...
```python
def is_good_draft_model(draft_model, target_model, eval_data):
    # Criterion 1: Fast enough (under 15% of target time)
    draft_time = benchmark_latency(draft_model)
    target_time = benchmark_latency(target_model)
    if draft_time > target_time * 0.15:
        return False

    # Criterion 2: High enough acceptance rate (at least 50%)
...
class TreeSpeculativeDecoder:
    ...
    def generate_speculation_tree(self, context):
    ...
    def verify_tree(self, tree, context):
...
<Callout type="warning" title="Batch size matters">
...
def optimize_temperature_for_speculation(
    draft_model, target_model, eval_prompts, target_temperature
):
    best_acceptance = 0
...
Fine-tune draft model on target domain for 5-15% higher acceptance rate:
...
def create_distillation_dataset(target_model, domain_prompts):
    ...
    for prompt in domain_prompts:
        ...
        dataset.append({'input_ids': prompt, 'labels': output})
    return dataset
...
Speculative decoding trades compute for latency, achieving 1.5-2.5x speedup for single-request inference. Key success factors: a fast draft model (under 15% of target time), high acceptance rate (over 70% for meaningful speedup), batch size 1 (otherwise already compute-bound), and domain matching between draft and target. For batch inference or throughput-focused deployments, stick with standard decoding.
